...
x0_reinforce = r.uniform(low=0.2,high=3.0,size=N)
sub_episode_length = 25 #number of time_steps in a sub-episode. 
...
ga_init_population = r.uniform(low=0.0, high=1.0, size=(pop_size,N))
pso_init_population = r.uniform(low=0.0, high=1.0, size=(pop_size,N))
...
generation_num = 20000  #number of theta updates for REINFORCE-IP, also serves as the number
...
lr = lr_schedule(initial_lr=0.00001, C=50000)   
...
actor_net = actor_distribution_network.ActorDistributionNetwork(   
                                         ...
                                         fc_layer_params=(18,18,18), #Hidden layers
...
for n in range(0,update_num):
    ...
    if n%eval_intv==0:
        ...
        print('mse(m1,best_solution of this generation)=', mse(m1,best_step.numpy()))
        print('mse(m2,best_solution of this generation)=', mse(m2,best_step.numpy()))
        ...
        print('act_std:', actions_distribution.stddev()[0,1]  )
        print('act_mean:', actions_distribution.mean()[0,1] ) #second action mean
    ...
    if best_step_reward>final_reward:
        ...
        print('mse(m1,updated final_solution)=', mse(m1,final_solution))
        print('mse(m2,updated final_solution of this generation)=', mse(m2,final_solution))

############# 2nd line in Table 3 (REINFORCE-OPT) 
print('final_solution=',final_solution,
      'final_reward=',final_reward,
     )
print('mse(m1,final_solution)=', mse(m1,final_solution))
print('mse(m2,final_solution)=', mse(m2,final_solution))
...
for npm in num_parents_mating:
    for ct in crossover_type:
        for mpg in mutation_percent_genes:
            ...
            print('mse(m1,best_solution under current hyper-params)=', mse(m1,solution))
            print('mse(m2,best_solution under current hyper-params)=', mse(m2,solution))
...
############# 6th line in Table 3 (PSO)       
print("final solution:",final_solution)
print('mse(m1,final_solution of this generation)=', mse(m1,final_solution))
print('mse(m2,final_solution of this generation)=', mse(m2,final_solution))
print("fitness of the final solution under current hyper-params:", best_record)
print("best hyper-params:",best_option)
#print('history:',pso_logs)
