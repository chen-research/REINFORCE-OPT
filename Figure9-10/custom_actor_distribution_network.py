######### Modification - tf.keras.activations.relu is replaced with tf.math.tanh
#"""
#tf.
#The ActorDistributionNetwork consists of two parts, the _encoder network and the _projection_networks.
#Observation is fed to the _encoder to produce encodes, then encodes (consist of action mean and standard deviation) are fed to _projection_networks to output action_distributions.
#The original ActorDistributionNetwork uses relu as its activation function for the _encoder network, which may cause encodes to be zero and further cause inefficiency in training.
#So I change relu to identity.
#"""
...
class ActorDistributionNetwork(network.DistributionNetwork):
  ...
  def __init__(self,
               ...
               activation_fn=tf.math.tanh, #tf.identity, tf.math.tanh,# tf.identity,  #tf.keras.activations.relu,  #Modification
